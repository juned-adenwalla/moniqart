                                    ...
                                    <div class="col-lg-4">
                                        <form action="" method="post">
                                            <select style="height: 36px;" name="statustype"
                                                ...
                                                if ($type == 'open') { ?><option value="open" selected>Open</option>
                                                <?php }
                                                if ($type == 'pending') { ?><option value="pending" selected>Pending
                                                </option>
                                                <?php }
                                                if ($type == 'resolved') { ?><option value="resolved" selected>Resolved
                                                </option><?php }
                                                if ($type == 'closed') { ?><option value="closed" selected>Closed
                                                </option>
                                                <?php }
                                                if ($type != 'open') { ?><option value="open">Open</option><?php }
                                                if ($type != 'pending') { ?><option value="pending">Pending</option>
                                                <?php }
                                                if ($type != 'resolved') { ?><option value="resolved">Resolved</option>
                                                <?php }
                                ...
                                <div class="row">
                                    <div class="col-lg-3" style="margin-bottom: 5px;">
                                        <i style="font-size: 18px"
                                            class="mdi mdi-calendar text-lg text-primary"></i>&nbsp;&nbsp;<?php echo date("F j, Y", strtotime(_getsinglticket($_ticid, 'CreationDate'))); ?>
                                    </div>
                                    <div class="col-lg-3" style="margin-bottom: 5px;">
                                        <i style="font-size: 18px"
                                            class="mdi mdi-account-circle text-primary"></i>&nbsp;&nbsp;<span
                                            style="font-size: 14px"> <?php echo _getsinglticket($_ticid, '_useremail'); ?></span>
                                    </div>
                                    <div class="col-lg-3">
                                        <a type="button" data-bs-toggle="modal" data-bs-target="#exampleModal">
                                            <i style="font-size: 18px"
                                                class="mdi mdi-reply text-primary"></i>&nbsp;&nbsp;Leave Reply
                                        </a>
                                    </div>
                                    <?php if (_getsinglticket($_ticid, '_image') != '') { ?>
                                    <div class="col-lg-3">
                                        <a href="../uploads/tickets/<?php echo _getsinglticket($_ticid, '_image'); ?>"><i
                                                style="font-size: 18px"
                                                class="mdi mdi-cloud-download text-primary"></i>&nbsp;&nbsp;Download
                                            Attachment</a>
                                    </div>
                                    <?php } ?>
                                </div>
